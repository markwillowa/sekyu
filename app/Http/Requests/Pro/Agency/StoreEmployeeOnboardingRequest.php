...
class StoreEmployeeOnboardingRequest extends FormRequest
{
    ...
    {
        return auth('pro_agency')->check();
    }

    public function rules(): array
    {
        $agency = auth('pro_agency')->user()->agency;
        $eligibleGuardProfileIds = $agency
            ? $agency->acceptedApplicantProfiles()->pluck('guard_profiles.id')->all()
            : [];
        ...
        return [
            'guard_profile_id' => [
                'required',
                'integer',
                Rule::in($eligibleGuardProfileIds),
            ],

            'employee_no' => ['nullable', 'string', 'max:255', Rule::unique('employees', 'employee_no')],
